...
public class SuperSocketMaster implements Runnable {
  ...
  private Vector<SuperSocketListen> portconnections = new Vector<SuperSocketListen>();
    ...
    if (blnServerMode == true) {
      ...
      while (true) {
        ...
        try {
          ...
          SuperSocketListen singleconnection = new SuperSocketListen(inTextField, outBuffer, inBuffer, socketObject.toString(), this);
          portconnections.addElement(singleconnection);
          inTextField.setText("CONN - "+singleconnection.strConnection);
  ...
  public class SuperSocketListen implements Runnable {
    ...
    public SuperSocketListen(JTextField incomingTxt, PrintWriter outgoing, BufferedReader incoming, String strConn, SuperSocketMaster ssm) {
      inTxt = incomingTxt;
      out = outgoing;
      in = incoming;
    ...
    public void run() {
      ...
      while (strLine != null) {
        try {
          strLine = in.readLine();
          if (strLine != null) {
            //System.out.println("Recieving Client Message: " + strLine);
